...
function playBingo(bingoCard) {
    let indexArrayX = {};
    let indexArrayY = {};
    const cardLength = bingoCard.length;
    for(let k = 0; k < numberToDraw.length; k++) {
        for (let i = 0; i < cardLength; i++) {
            const cardRow = bingoCard[i];
            const cardRowIndex = cardRow.indexOf(numberToDraw[k]);
            if(cardRowIndex !== -1) {
                indexArrayX[i] = indexArrayX[i] ? indexArrayX[i]+cardRowIndex.toString() : cardRowIndex.toString();
                indexArrayY[cardRowIndex] = indexArrayY[cardRowIndex] ? indexArrayY[cardRowIndex]+i.toString() : i.toString();

                // remove number from cardRow, So it will be easy to calculate sum.
                cardRow.splice(cardRowIndex, 1, '*');

                if(indexArrayX[i].length === 5 ) {
                    return [k, parseInt(numberToDraw[k]) * calculateSum(bingoCard)];
                }
                if(indexArrayY[cardRowIndex].length === 5 ) {
                    return [k, parseInt(numberToDraw[k]) * calculateSum(bingoCard)];
                }
            }
        }
...
function startBingo(bingoCard) {
    let bingoCardReady = [];
    let allPlayersRecord = [];
    for (let k = 0; k < bingoCard.length; k++) {
        if (bingoCard[k].length === 1) {
            bingoCardReady = [];
        }
        if (bingoCard[k].length > 1 && bingoCardReady.length < 6) {
            bingoCardReady.push(bingoCard[k]);
        }
        if (bingoCardReady.length === 5) {
            allPlayersRecord.push(playBingo(bingoCardReady));
        }
    }
    allPlayersRecord.sort(function(a,b) {
        return a[0]-b[0]
    });
    return allPlayersRecord;
}

const result = startBingo(mat);
console.log( ' Solution for 1 =>', result[0]);
console.log( ' Solution for 2 =>', result.slice(-1).pop());